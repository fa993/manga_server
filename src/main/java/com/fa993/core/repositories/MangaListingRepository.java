...
public interface MangaListingRepository extends JpaRepository<MangaListing, Integer> {
    ...
    @Query(
            value = "select manga_listing.public_id as publicId, manga_listing.name as name, manga_listing.cover_url as coverURL, manga_listing.description_small as smallDescription, manga_listing.genres as genres from manga, manga_listing, manga_genre where manga.manga_id = manga_genre.manga_id AND manga.manga_id = manga_listing.manga_id AND manga_genre.genre_id IN :genres AND manga.is_main = 1 group by manga.manga_id HAVING count(*) = :size order by manga.name ASC limit :offset, :limit",
            nativeQuery = true
    )
    ...
    //TODO test a self join query to capture genre in different sources
    @Query(
            value = "select manga_listing.public_id as publicId, manga_listing.name as name, manga_listing.cover_url as coverURL, manga_listing.description_small as smallDescription, manga_listing.genres as genres from manga, manga_listing where manga.is_main = 1 AND manga.manga_id = manga_listing.manga_id  order by manga.name ASC limit :offset, :limit",
            nativeQuery = true
    )
